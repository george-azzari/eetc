...
def getS1Plus(poly, year, correctlia, addspeckle):
    ...
    s1obj = Sentinel1(poly, startdate, enddate)

    # NOTE: it is risky to set the orbit to None here; combining asc/desc orbits can generate a lot of noise.
    s1 = s1obj.get_img_coll(correctlia=correctlia, addbands=True, addspeckle=addspeckle, addtexture=False, orbit=None)

    # Add DOY band and convert backscatter to Natural.
    s1 = s1.map(lambda img: imgtools.addDOY(img))
    s1 = s1.map(lambda img: s1obj.replace_db_bands(img, ''))
...
def seasonMedians(imgcoll, bandnames, year, asimgcoll, addtexture, glcmvars):
    ...
    if asimgcoll:
        ...
        medcomp = renameBands(bim0, 'S1').addBands(renameBands(bim1, 'S2')).addBands(renameBands(bim2, 'S3'))\
            .set('year', year)
        ...
        if addtexture:
            ...
            if glcmvars is not None:
                selbands = ee.List(glcmvars).map(lambda n: getNewNames(medcomp.bandNames(), n)).flatten()
